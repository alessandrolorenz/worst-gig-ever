# V2 Production — Vocalist Pack

Read `00-style-bible.md`, then use the approved canonical reference
`design-reference/m14-v2-proof/vocalist-reference-v2.png` as **Image 1** on
...
Original fictional adult; medium height, compact wiry build; angular triangular
face with sharp eyebrows; messy teal crest with one shaved side; mustard
sleeveless ripped vest; magenta shirt; black narrow jeans with one magenta
repair patch; white high-top boots; simple black wrist pieces; generic wired
microphone with cable. Tense, defiant, comically overcommitted — never violent.
...
|---|---|---|
| `vocalist_idle.png` | `idle` | Canonical ambient pose. Three-quarter **back** toward the crowd, as Pack 1 established — the vocalist stands between the drummer and the audience. Weight on one leg, mic up, shoulders squared at the crowd. |
| `vocalist_loop_a.png` | `loopA` | Edit of idle. Head tips toward the mic, mic hand rises slightly, far shoulder drops. Feet unchanged. |
| `vocalist_loop_b.png` | `loopB` | Edit of idle. Opposite small beat: head back, free arm bent higher, torso leans a few degrees. Feet unchanged. |
| `vocalist_blocking.png` | `blocking` | Turns and leans **back toward the drummer/camera**, intruding into the sightline. Face fully visible and comically self-absorbed — this is the state the joke depends on, so the expression must read at phone size. |
| `vocalist_hit_reaction.png` | `hitReaction` | Comic surprise/stumble from a strike. Larger movement allowed; identity, outfit, and palette persist. No injury, no gore. |
| `vocalist_dodge.png` | `dodge` | Clear duck or lateral shift out of the projectile corridor. Reads as a duck in silhouette alone. |
